        ...
        <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="nama_menu" name="nama_menu" value="{{ old('nama_menu') }}" class="w-full px-3 py-2.5 text-sm border rounded-lg outline-none {{ $errors->has('nama_menu') ? 'border-red-400 bg-red-50' : 'border-zinc-300 focus:border-zinc-500 focus:ring-2 focus:ring-zinc-100' }}" placeholder="Espresso, Caramel Macchiato..." required>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="harga" name="harga" value="{{ old('harga') }}" min="0" class="w-full px-3 py-2.5 text-sm border rounded-lg outline-none tabular-nums {{ $errors->has('harga') ? 'border-red-400 bg-red-50' : 'border-zinc-300 focus:border-zinc-500 focus:ring-2 focus:ring-zinc-100' }}" placeholder="18000" required>
                    ...
                    <div>
                        ...
                        <input type="number" id="stok" name="stok" value="{{ old('stok', 0) }}" min="0" class="w-full px-3 py-2.5 text-sm border rounded-lg outline-none tabular-nums {{ $errors->has('stok') ? 'border-red-400 bg-red-50' : 'border-zinc-300 focus:border-zinc-500 focus:ring-2 focus:ring-zinc-100' }}" required>
                        @error('stok')<p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>@enderror
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-zinc-700 mb-1.5">Foto Produk <span class="text-zinc-400 font-normal">(opsional, maks. 2MB)</span></label>
                    <div
                        id="drop-zone"
                    ...
                    >
                        <div id="preview-wrap" class="hidden w-full flex justify-center">
                            <img id="preview-img" src="" alt="Preview" class="max-h-48 rounded-lg object-contain shadow-sm">
                        </div>
                        <div id="upload-placeholder" class="flex flex-col items-center gap-1 text-center">
                            ...
                            <p class="text-sm text-zinc-500">Klik untuk pilih foto</p>
                            <p class="text-xs text-zinc-400">JPG, PNG, WebP · maks. 2MB</p>
                        </div>
                        <p id="file-name" class="hidden text-xs text-zinc-500 mt-1"></p>
                    </div>
